	      ...
	      const struct blk_holder_ops *hops, struct file *bdev_file)
{
	...
	fstore_register_map(uuid_end, "bio", -1, 0, /*offsetof(struct bio, fstore_scratch), sizeof(((struct bio*) NULL)->fstore_scratch),*/ &bdev->fstore_end_times, 16);

	fstore_uuid_t uuid_queued_reads;
	uuid_queued_reads.strs[0] = dev_name(&bdev->bd_device);
	uuid_queued_reads.strs[1] = "__queued_reads";
	fstore_register_map(uuid_queued_reads, "request_queue", -1, 0, /* offsetof(struct request_queue, fstore_scratch), sizeof(((struct request_queue*) NULL)->fstore_scratch),*/ &bdev->fstore_queued_reads, 16);

	fstore_uuid_t uuid_queue_ss;
	uuid_queue_ss.strs[0] = dev_name(&bdev->bd_device);
	uuid_queue_ss.strs[1] = "__queue_ss";
	fstore_register_map(uuid_queue_ss, "bio", -1, 0, /*offsetof(struct bio, fstore_scratch), sizeof(((struct bio*) NULL)->fstore_scratch),*/ &bdev->fstore_queue_ss, 16);

	fstore_uuid_t uuid_sectors;
	uuid_sectors.strs[0] = dev_name(&bdev->bd_device);
	uuid_sectors.strs[1] = "__sectors";
	fstore_register_map(uuid_sectors, "bio", -1, 0, /*offsetof(struct bio, fstore_scratch), sizeof(((struct bio*) NULL)->fstore_scratch),*/ &bdev->fstore_sectors, 16);
