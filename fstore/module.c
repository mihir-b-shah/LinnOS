...
int fstore_register_map(fstore_uuid_t id, const char* key_id, int scratch_offs, unsigned scratch_sz, fstore_map_ptr_t* map, int n_past_track) {
	...
	if (key_infos[key_i].locs[loc_i].sz >= sizeof(fstore_val_type_t)) {
		printk(KERN_INFO "using fast lookup %s:%s %s:%s:%d:%d:%s:%d:%d.\n", id.strs[0], id.strs[1], key_id, key_infos[key_i].id, key_infos[key_i].base_offs, key_infos[key_i].base_sz, key_infos[key_i].locs[loc_i].id, key_infos[key_i].locs[loc_i].offs, key_infos[key_i].locs[loc_i].sz);
